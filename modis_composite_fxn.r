...
##of the normalized difference vegetation index (NDVI) from a time series of daily MODIS data
##This function is intended to be applied to a timeseries from a single pixel and requires a row for each day;
##if the timeseries has been filtered (for clouds, tides, etc), leave NA in the NDVI column for the filtered day
...
##Selects data with the smallest view angles within the specified window through the following rules: 
##  -if available, create an average of up to 5 observations with view zenith angles <40 degrees, 
##  -if obs with viewzen <40 don't exist but viewzen <55 exists, use the single minimum view zen NDVI value, 
##  -insert NA for windows with no view zenith angles <55 degrees
##Usage, data is a dataframe with the columns "date", "viewzen", "NDVI", 
##       window is the desired length in days of the compositing window

slidefx <- function(data, window){
    ...
    years<-unique(as.numeric(format(data$date, "%Y")))
    ...
    result <- matrix(nrow = nrow(dates),ncol=4,
                     dimnames=list(seq(1:nrow(dates)), c("date", "doy","viewzen", "NDVI")))
    for(i in 1:nrow(dates)){
        ...
        if(sum(complete.cases(h))>0){
            ...
            if(length(good<-which(hordered$viewzen<40))>0){
                ...
                result[i,4]<-mean(hordered$NDVI[good][1:5],na.rm=T)
            }else{
                if(min(h$viewzen, na.rm=T)<55){
                    ##grab the one minimum view zen NDVI value, if viewzen <40 does not exist but viewzen <55 does
                    result[i,c(1,2,3)] <-c(dates$date[i],dates$days[i], 
                                           hordered$viewzen[1])
                    result[i,4]<-hordered$NDVI[1]}
            }
        }else{
            ##is all view zens are high, than NA
